...
def process_data(data_list):
    # Step 1: Group data_list by 'cate'
    grouped_data = itertools.groupby(data_list, key=lambda x: x['cate'])
    # 创建一个字典存储分组结果
    grouped_lists = {}

    # 遍历分组结果，并将每个分组存储到对应的键中
    for key, group in grouped_data:
        grouped_lists[key] = list(group)
    ...
    for item in temp1:
        cate = item['cate']
        if cate not in result_dict1:
            result_dict1[cate] = []
        result_dict1[cate].append(item)
    result_dict2 = {}
    for item in temp2:
        cate = item['cate']
        if cate not in result_dict2:
            result_dict2[cate] = []
        result_dict2[cate].append(item)
    new_list1 = []
    list_len1 = max([len(v) for v in result_dict1.values()])
    for i in range(list_len1):
        # 遍历每个键对应的值列表，取出第 i 个元素
        for key, value in result_dict1.items():
            if i < len(value):
                new_list1.append(value[i])

    new_list2 = []
    list_len2 = max([len(v) for v in result_dict2.values()])
    for i in range(list_len2):
        # 遍历每个键对应的值列表，取出第 i 个元素
        for key, value in result_dict2.items():
            if i < len(value):
                new_list2.append(value[i])
...
def set_logger(log_file, name="default"):
    ...
    # 生成带有日期和时间的文件名
    log_file = f"simulator-{time_str}.log"
